...
class ConcurrentTrueOnlineSarsaLambdaAgent:
    ...
    def __init__(self, num_real_actions, num_actions, num_state_features,
                 gamma=0.999, base_alpha=0.25, lambda_=0.5):
        ...
        self.theta = np.ones(self.dim)   # TODO different initialization
    ...
    def choose_action_eps_greedy(self, state_features, card_id, t, transaction_date, fraud, epsilon=0.1):
        if np.random.random_sample() < epsilon:
            action = int(np.random.random_sample() * self.num_actions)
        else:
            best_q = -1_000_000.0
            best_actions = []
    ...
    def on_customer_leave(self, card_id, customer):     # TODO can be different fraudster + genuine with same card ID
